...
export default function PreStudy() {
  ...
  if (status === "success") return <section className="study" id="preestudio"><div className="shell success-panel" role="status"><div className="kicker mono">Preestudio completado</div><h2>La solicitud ya tiene contexto técnico.</h2><p>Referencia <strong>{reference}</strong>. Conserva este código para cualquier consulta.</p><button className="v21-btn darkbtn" type="button" onClick={() => {setData(initialPreStudyData);setStep(1);setStatus("idle");}}>Nuevo preestudio</button></div></section>;
